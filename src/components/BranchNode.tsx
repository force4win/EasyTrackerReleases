...
export const BranchNode = memo(({ id, data, selected }: NodeProps<BranchCustomNode>) => {
  return (
    <div
      className={`group relative px-7 py-4 rounded-full text-white font-bold text-sm shadow-2xl min-w-[160px] text-center select-none transition-all duration-200 fa-glass-panel overflow-hidden border ${
        selected
    ...
    >
      ...
      <div className="flex flex-col items-center justify-center relative z-10 space-y-1">
        <span className="fa-text-shadow tracking-wide text-base">{data.label}</span>
        <span className="text-[10px] font-mono text-cyan-200 bg-cyan-950/80 px-3 py-0.5 rounded-full border border-cyan-400/30 uppercase tracking-wider font-semibold shadow-inner">
          {data.branchType}
        </span>
      </div>
